...
const open = args => {
  ...
  const createNotice = notify => {
    notify.createNotice({
      key: target,
      content: (
        <div className={styles[`${prefixCls}-with-icon`]}>
          <div className={styles[`${prefixCls}-icon`]}>
            <Icon type={IconMap[type] || 'check'} style={{ fontSize: 20 }} />
          </div>
          <div className={styles[`${prefixCls}-text`]}>
            {title && <div className={styles[`${prefixCls}-title`]}>{title}</div>}
            {content && <div className={styles[`${prefixCls}-content`]}>{content}</div>}
          </div>
          {btns ? <div className={styles[`${prefixCls}-btns`]}>{btns}</div> : null}
        </div>
      ),
      ...newOptions,
    })
