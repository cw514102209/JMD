...
export default class Alert extends PureComponent {
  ...
  renderIcon() {
    ...
    return (
      <Icon
        className={classnames(styles.jm_alter_icon, {
          [styles['jm_alter_large']]: message && description
        })}
        theme="filled"
        type={iconType}
  ...
  render() {
    ...
    return (
      <div className={classnames(styles.jm_alter_wrapper, styles[`jm_alter_${type}`], { [styles['jm_alter_hidden']]: hidden }, className)}>
        {showIcon && this.renderIcon()}
        <div className={styles.jm_alter_description}>
          {message && <h3>{message}</h3>}
          <span>{description}</span>
        </div>
        {closable && (
          <div className={styles.jm_alter_close} onClick={this.onHidden.bind(this)}>
