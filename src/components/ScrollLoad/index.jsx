...
export default class ScrollLoad extends React.Component {
  ...
  renderContent() {
    ...
    if (this.props.isEmpty || isEmpty(data) || !children) {
      return empty || <Empty className={styles.jm_scrollload_empty} desc="暂无数据" />
    }
  ...
  render() {
    ...
    return (
      <div className={classnames(styles.jm_scrollload_wrapper, wrapperClassName)}>
        <Spin spinning={loading}>
          <div
            ref={this.containerRef}
            className={styles.jm_scrollload_main}
            style={allStyles}
            onScrollCapture={this.handleScroll}
          >
            <div
              className={classnames(styles.jm_scrollload_content, className)}
              style={{ minHeight: minHeight || height }}
            >
              {this.renderContent()}
            </div>
            {loadMore && (
              <div className={styles.jm_scrollload_loadMore}>
                <Spin />
              </div>
            )}
          </div>
